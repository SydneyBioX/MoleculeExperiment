...
6) Add boundaries from an external segmentation algorythm.

In this example, we use the exent of the molecules of generated for `toyME` to 
allign the boundaries with the molecules. In general, the extent of the 
segmentation is required for this allignment.
...
# boundary data has not been included in the package extdata.
...
g <- ggplot() +
  geom_point_me(
    small_me,
    assayName = "detected", byColour = "feature_name", size = 0.2
  ) +
  geom_polygon_me(
    small_me,
