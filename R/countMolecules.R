...
.count_molecules_boundaries_v2 <- function(object,
                                           molecules_assay,
                                           boundaries_assay,
                                           matrix_only = FALSE) {
  # messages from getters will notify user about assays chosen
  ...
  stopifnot(identical(
    names(init_mols[[molecules_assay]]), names(init_bds[[boundaries_assay]])
  ))
  ...
  xvalsList <- lapply(
    samples, function(x) vector(mode = "list", length = length(features))
  )
  ivalsList <- lapply(
    xvalsList, function(x) lapply(x, function(y) rep(NULL, length(y)))
  )
  jnamesList <- lapply(
    xvalsList, function(x) lapply(x, function(y) rep(NULL, length(y)))
  )
  ...
  # suppress messages to avoid repeating getter messages
  bds_all_flat <- suppressMessages(MoleculeExperiment::boundaries(object,
    assayName = boundaries_assay,
    flatten = TRUE
  ))

  for (sample in samples) {
    bds_df <- bds_all_flat[bds_all_flat$sample_id == sample, ]
    factors <- interaction(bds_df$sample_id, bds_df$segment_id)
    factors_int <- as.integer(factors)
    bds_levels <- levels(factors)
    bds_mat <- as.matrix(cbind(
    ...
    for (feature in features) {
      ...
      out <- terra::relate(bds, mols, "covers", pairs = TRUE)
      xvals <- as.data.table(out)[, .(N = ifelse(is.null(`.BY`[[1]]), NA, .N)), by = list(V1 = out[, 1])][, N]
      ivals <- rep(which(features == feature), length(xvals))
      jvals <- unique(out[, 1])
      jnames <- bds_levels[jvals]
